...
foreach ($_POST['id_nrocuenta'] as $key => $value) {
    ...
    if ($contador != 1) {
        ...
        $stmt = $pdo->prepare('INSERT INTO tb_detalle_cheque (id_nrocuenta,nro_ci,fecha_ci,nro_ce,fecha_ce,nro_cheque,fecha_emision_cheque,monto,
        nro_envio,fecha_aprobado,fecha_entregado,fecha_pagado,observacion,id_usuario,nt,id_anio_nt,fyh_creacion) 
    VALUES (:id_nrocuenta,:nro_ci,:fecha_ci,:nro_ce,:fecha_ce,:nro_cheque,:fecha_emision_cheque,:monto,:nro_envio,
        :fecha_aprobado,:fecha_entregado,:fecha_pagado,:observacion,:id_usuario,:nt,:id_anio_nt,:fyh_creacion)');
