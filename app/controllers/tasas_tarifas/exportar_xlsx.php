...
//merge heading
$spreadsheet->getActiveSheet()->mergeCells("B1:T1");
...
$spreadsheet->getActiveSheet()
    ...
    ->setCellValue('K2', "CUENTA")
    ->setCellValue('L2', "RESOLUCION RECTORAL")
    ->setCellValue('M2', "ARCHIVO")
    ->setCellValue('N2', "VIGENCIA")
    ->setCellValue('O2', "SITUACION")
    ->setCellValue('P2', "TIPO")
    ->setCellValue('Q2', "OBSERVACION")
    ->setCellValue('R2', "FECHA REGISTRO")
    ->setCellValue('S2', "FECHA ACTUALIZACION")
    ->setCellValue('T2', "USUARIO");

//set font style and background color
$spreadsheet->getActiveSheet()->getStyle('A2:T2')->applyFromArray($tableHead); //estilo de fondo en la cabecera
$spreadsheet->getActiveSheet()->getStyle('A2:T2')->getAlignment()->setHorizontal('center'); //alineacion cental
...
$spreadsheet->getActiveSheet()->getStyle('A2:T2')->applyFromArray($styleArray);
...
if (!isset($tasas_tarifas_datos)) {
    ...
    foreach ($tasas_tarifas_datos as $tasas_tarifas_dato) {
        ...
        $spreadsheet->getActiveSheet()
            ...
            ->setCellValue('K' . $row, $tasas_tarifas_dato['cta'])
            ->setCellValue('L' . $row, $tasas_tarifas_dato['resolucion'])
            ->setCellValue('M' . $row, $tasas_tarifas_dato['archivo_resolucion'])
            ->setCellValue('N' . $row, $tasas_tarifas_dato['vigencia'])
            ->setCellValue('O' . $row, $tasas_tarifas_dato['situacion'])
            ->setCellValue('P' . $row, $tasas_tarifas_dato['categoria_transaccion'])
            ->setCellValue('Q' . $row, $tasas_tarifas_dato['observacion'])
            ->setCellValue('R' . $row, $tasas_tarifas_dato['fyh_creacion_tyt'])
            ->setCellValue('S' . $row, $tasas_tarifas_dato['fyh_actualizacion_tyt'])
            ->setCellValue('T' . $row, $tasas_tarifas_dato['nombres'] . " " . $tasas_tarifas_dato['apaterno'] . " " . $tasas_tarifas_dato['amaterno']);
        ...
        if ($row % 2 == 0) {
            //even row
            $spreadsheet->getActiveSheet()->getStyle('A' . $row . ':T' . $row)->applyFromArray($evenRow);
            $spreadsheet->getActiveSheet()->getStyle('A' . $row . ':T' . $row)->applyFromArray($styleArray);
            $spreadsheet->getActiveSheet()->getStyle('A' . $row . ':T' . $row)->getAlignment()->setHorizontal('left');
        } else {
            //odd row
            $spreadsheet->getActiveSheet()->getStyle('A' . $row . ':T' . $row)->applyFromArray($oddRow);
            $spreadsheet->getActiveSheet()->getStyle('A' . $row . ':T' . $row)->applyFromArray($styleArray);
            $spreadsheet->getActiveSheet()->getStyle('A' . $row . ':T' . $row)->getAlignment()->setHorizontal('left');
        }
        //increment row
    ...
    $lastRow = $row - 1;
    //set the autofilter
    $spreadsheet->getActiveSheet()->setAutoFilter("A" . $firstRow . ":T" . $lastRow); //Establecer filtros en las columnas
}
