...
$spreadsheet->getActiveSheet()
    ...
    ->setCellValue('O2', "VIGENCIA")
    ->setCellValue('P2', "SITUACION")
    ->setCellValue('Q2', "TIPO")
    ->setCellValue('R2', "ESTADO")
    ->setCellValue('S2', "OBSERVACION")
    ->setCellValue('T2', "FECHA REGISTRO")
    ->setCellValue('U2', "FECHA ACTUALIZACION")
    ->setCellValue('V2', "USUARIO");

//set font style and background color
$spreadsheet->getActiveSheet()->getStyle('A2:V2')->applyFromArray($tableHead); //estilo de fondo en la cabecera
$spreadsheet->getActiveSheet()->getStyle('A2:V2')->getAlignment()->setHorizontal('center'); //alineacion cental
...
$sql_tasas_tarifas = "SELECT tyt.id_tasas_tarifas as id_tasas_tarifas, 
    ...
    INNER JOIN tb_estado_tasas as estado_tasas ON estado_tasas.id_estado_tasas = tyt.id_estado 
    WHERE tyt.visible!=1 ";
...
if (isset($_SESSION['busqueda_boton_tasas_tarifas'])) {
    if (
        ...
        if (!empty($resolucion)) {
            $sql_tasas_tarifas .= " AND tyt.resolucion='" . $resolucion . "'";
        }
...
if (!isset($tasas_tarifas_datos)) {
    ...
    foreach ($tasas_tarifas_datos as $tasas_tarifas_dato) {
        ...
        $spreadsheet->getActiveSheet()
            ...
            ->setCellValue('O' . $row, $tasas_tarifas_dato['vigencia'])
            ->setCellValue('P' . $row, $tasas_tarifas_dato['situacion'])
            ->setCellValue('Q' . $row, $tasas_tarifas_dato['categoria_transaccion'])
            ->setCellValue('R' . $row, $tasas_tarifas_dato['estado'])
            ->setCellValue('S' . $row, $tasas_tarifas_dato['observacion'])
            ->setCellValue('T' . $row, $tasas_tarifas_dato['fyh_creacion_tyt'])
            ->setCellValue('U' . $row, $tasas_tarifas_dato['fyh_actualizacion_tyt'])
            ->setCellValue('V' . $row, $tasas_tarifas_dato['nombres'] . " " . $tasas_tarifas_dato['apaterno'] . " " . $tasas_tarifas_dato['amaterno']);
        ...
        if ($row % 2 == 0) {
            ...
            $spreadsheet->getActiveSheet()->getStyle('A' . $row . ':V' . $row)->applyFromArray($styleArray);
            $spreadsheet->getActiveSheet()->getStyle('A' . $row . ':V' . $row)->getAlignment()->setHorizontal('left');
        } else {
            //odd row
    ...
    $lastRow = $row - 1;
    //set the autofilter
    $spreadsheet->getActiveSheet()->setAutoFilter("A" . $firstRow . ":V" . $lastRow); //Establecer filtros en las columnas
}
