...
$sql_tasas_tarifas = "SELECT tyt.id_tasas_tarifas as id_tasas_tarifas, 
    ...
    LEFT JOIN tb_detalle_tyt as detalle_tyt ON (detalle_tyt.resolucion = tyt.resolucion AND detalle_tyt.archivo = tyt.archivo AND detalle_tyt.monto = tyt.monto AND detalle_tyt.codigo_pago = tyt.codigo_pago AND detalle_tyt.modalidad = tyt.modalidad AND detalle_tyt.concepto = tyt.concepto AND detalle_tyt.referencia = tyt.referencia AND detalle_tyt.clasificador = tyt.clasificador AND detalle_tyt.codigo_facultad = tyt.codigo_facultad AND detalle_tyt.dependencia = tyt.dependencia AND detalle_tyt.codigo_ser_banco = tyt.codigo_ser_banco AND detalle_tyt.cta = tyt.cta AND detalle_tyt.vigencia = tyt.vigencia AND detalle_tyt.situacion = tyt.situacion) 
...
class PDF extends FPDF
{
    public function header()
    {
        ...
        $this->SetY(65);
        $this->SetFont('Arial', 'B', 8); //fuente
        $this->Cell(w: 38); //mover a la derecha
        $this->Cell(40, 10, utf8_decode('DEPEND.')); //texto

        $this->SetY(65);
        $this->SetFont('Arial', 'B', 8); //fuente
        $this->Cell(w: 100); //mover a la derecha
        $this->Cell(40, 10, utf8_decode('CONCEPTO')); //texto

        $this->SetY(65);
        $this->SetFont('Arial', 'B', 8); //fuente
        $this->Cell(174); //mover a la derecha
        ...
        $this->SetY(65);
        $this->SetFont('Arial', 'B', 8); //fuente
        $this->Cell(264); //mover a la derecha
...
if (!isset($tasas_tarifas_datos)) {
    ...
    foreach ($tasas_tarifas_datos as $tasas_tarifas_dato) {
        ...
        */
        ...
        $pdf->Cell(12, 5, utf8_decode($i));

        $pdf->Cell(8, 5, utf8_decode($tasas_tarifas_dato["codigo_pago"]),null,null,'R');
        ...
        $pdf->Cell(15, 5, utf8_decode($importe_tarifa),null,null,'R');
    ...
    switch($tasas_tarifas_dato["dependencia"]){
        ...
        case "FACULTAD DE INGENIERIA DE GEOGRAFICA, AMBIENTAL Y ECOTURISMO": $tasas_tarifas_dato["dependencia"]="FIGAE";break;
        ...
        $pdf->Cell(17, 5, utf8_decode($tasas_tarifas_dato["dependencia"]),null,null,'R');
        ...
        $pdf->Cell(25, 5, utf8_decode($tasas_tarifas_dato["cta"]),null,null,'R');
        ...
        $pdf->Cell(70, 5, utf8_decode($tasas_tarifas_dato["resolucion"]),null,null,'R');
