...
$fileName = "REPORTE DE ESTADO DE EGRESOS " . date('d-m-Y H:i:s') . ".xlsx";
...
//merge heading
$spreadsheet->getActiveSheet()->mergeCells("B1:AR1");
...
$spreadsheet->getActiveSheet()
    ...
    ->setCellValue('D2', "SALDO INICIAL")
    ->setCellValue('E2', "PERIODO")
    ->setCellValue('F2', "ESTADO")
    ->setCellValue('G2', "OBSERVACION EGRESO")
    ->setCellValue('H2', "NT")
    ->setCellValue('I2', "AÑO NT")
    ->setCellValue('J2', "PROVEIDO CONTABILIDAD")
    ->setCellValue('K2', "FECHA CONTABILIDAD")
    ->setCellValue('L2', "PROVEIDO DIGA")
    ->setCellValue('M2', "FECHA DIGA")
    ->setCellValue('N2', "OFICIO")
    ->setCellValue('O2', "FECHA OFICIO")
    ->setCellValue('P2', "ASUNTO")
    ->setCellValue('Q2', "DESCRIPCION INFORME")
    ->setCellValue('R2', "N° INFORME")
    ->setCellValue('S2', "FECHA INFORME")
    ->setCellValue('T2', "RESOLUCION")
    ->setCellValue('U2', "FECHA RESOLUCION")
    ->setCellValue('V2', "ESPECIALIDAD")
    ->setCellValue('W2', "DETALLE")
    ->setCellValue('X2', "PERIODO")
    ->setCellValue('Y2', "NRO ORDEN COMPRA")
    ->setCellValue('Z2', "NRO ORDEN SERVICIO")
    ->setCellValue('AA2', "SIAF")
    ->setCellValue('AB2', "MONTO")
    ->setCellValue('AC2', "TOTAL EGRESOS")
    ->setCellValue('AD2', "TOTAL INGRESOS")
    ->setCellValue('AE2', "SALDO FINAL")
    ->setCellValue('AF2', "NOTA PAGO")
    ->setCellValue('AG2', "FECHA PAGO")
    ->setCellValue('AH2', "FECHA GIRO")
    ->setCellValue('AI2', "ESTADO GIRO")
    ->setCellValue('AJ2', "NRO ESTADO CTA")
    ->setCellValue('AK2', "FECHA ESTADO CTA")
    ->setCellValue('AL2', "INFORME INGRESO")
    ->setCellValue('AM2', "FECHA INFORME INGRESO")
    ->setCellValue('AN2', "DESCRIPCION ESTADO CTA")
    ->setCellValue('AO2', "OBSERVACION NT")
    ->setCellValue('AP2', "FECHA REGISTRO")
    ->setCellValue('AQ2', "FECHA ACTUALIZACION")
    ->setCellValue('AR2', "USUARIO");




//set font style and background color
$spreadsheet->getActiveSheet()->getStyle('A2:AR2')->applyFromArray($tableHead); //estilo de fondo en la cabecera
$spreadsheet->getActiveSheet()->getStyle('A2:AR2')->getAlignment()->setHorizontal('center'); //alineacion cental
...
$spreadsheet->getActiveSheet()->getStyle('A2:AR2')->applyFromArray($styleArray);
...
$sql_egresos = "SELECT egresos.id_egresos as id_egresos,
    ...
    detalle_egresos.total_egresos as total_egresos,
    detalle_egresos.especialidad as especialidad,
    ...
    detalle_egresos.estado_giro as estado_giro,
    detalle_egresos.asunto as asunto,
    detalle_egresos.descripcion as descripcion,
    detalle_egresos.informe as informe,
    detalle_egresos.fecha_informe as fecha_informe,
    detalle_egresos.informe_ingresos as informe_ingresos,
    detalle_egresos.fecha_informe_ingresos as fecha_informe_ingresos,
    detalle_egresos.numero_ec as numero_ec,
    detalle_egresos.fecha_ec as fecha_ec,
    detalle_egresos.descripcion_ec as descripcion_ec,
    detalle_egresos.periodo_meses as periodo_meses,
    ...
    detalle_egresos.saldo as saldo,
    ...
    LEFT JOIN tb_detalle_egresos as detalle_egresos ON (detalle_egresos.facultad=egresos.cargo_facultad AND detalle_egresos.actividad_principal=egresos.actividad_principal AND detalle_egresos.periodo=egresos.anio) 
...
if (!isset($egresos_datos)) {
    ...
    foreach ($egresos_datos as $egresos_dato) {
        ...
        if ($egresos_dato['proveido_contabilidad'] != null) {
            $egresos_dato['proveido_contabilidad'] = $egresos_dato['proveido_contabilidad'] . "-OC-UNFV";
        } else {
            $egresos_dato['proveido_contabilidad'] = "";
        }

        if ($egresos_dato['proveido_diga'] != null) {
            $egresos_dato['proveido_diga'] = $egresos_dato['proveido_diga'] . "-DIGA-UNFV";
        } else {
            $egresos_dato['proveido_diga'] = "";
        }

        if ($egresos_dato['informe'] != null) {
            $egresos_dato['informe'] = $egresos_dato['informe'] . "-" . $fecha_informe_anio . "-E-OT-DIGA-UNFV";
        } else {
            $egresos_dato['informe'] = "";
        }

        if ($egresos_dato['resolucion'] != null) {
            $egresos_dato['resolucion'] = $egresos_dato['resolucion'] . "-CU-UNFV";
        } else {
            $egresos_dato['resolucion'] = "";
        }

        if($egresos_dato['egresos']==0.00){
            $egresos_dato['egresos']=$egresos_dato['total_egresos'];
        }
        ...
        $spreadsheet->getActiveSheet()
            ...
            ->setCellValue('D' . $row, $egresos_dato['saldo_inicial'])
            ...
            ->setCellValue('J' . $row, $egresos_dato['proveido_contabilidad'])
            ->setCellValue('K' . $row, $egresos_dato['fecha_proveido_contabilidad'])
            ->setCellValue('L' . $row, $egresos_dato['proveido_diga'])
            ...
            ->setCellValue('P' . $row, $egresos_dato['asunto'])
            ->setCellValue('Q' . $row, $egresos_dato['descripcion'])
            ->setCellValue('R' . $row, $egresos_dato['informe'])
            ->setCellValue('S' . $row, $egresos_dato['fecha_informe'])
            ->setCellValue('T' . $row, $egresos_dato['resolucion'])
            ->setCellValue('U' . $row, $egresos_dato['fecha_resolucion'])
            ->setCellValue('V' . $row, $egresos_dato['especialidad'])
            ->setCellValue('W' . $row, $egresos_dato['detalle'])
            ->setCellValue('X' . $row, $egresos_dato['periodo_meses'])
            ->setCellValue('Y' . $row, $egresos_dato['nro_orden_compra'])
            ->setCellValue('Z' . $row, $egresos_dato['nro_orden_servicio'])
            ->setCellValue('AA' . $row, $egresos_dato['siaf'])
            ->setCellValue('AB' . $row, $egresos_dato['monto'])
            ->setCellValue('AC' . $row, $egresos_dato['egresos'])
            ->setCellValue('AD' . $row, $egresos_dato['ingresos'])
            ->setCellValue('AE' . $row, $egresos_dato['saldo'])
            ->setCellValue('AF' . $row, $egresos_dato['comprobante_pago'])
            ->setCellValue('AG' . $row, $egresos_dato['fecha_pago'])
            ->setCellValue('AH' . $row, $egresos_dato['fecha_giro'])
            ->setCellValue('AI' . $row, $egresos_dato['estado_giro'])
            ->setCellValue('AJ' . $row, $egresos_dato['numero_ec'])
            ->setCellValue('AK' . $row, $egresos_dato['fecha_ec'])
            ->setCellValue('AL' . $row, $egresos_dato['informe_ingresos'])
            ->setCellValue('AM' . $row, $egresos_dato['fecha_informe_ingresos'])
            ->setCellValue('AN' . $row, $egresos_dato['descripcion_ec'])
            ->setCellValue('AO' . $row, $egresos_dato['observacion_detalle'])
            ->setCellValue('AP' . $row, $egresos_dato['fyh_creacion_detalle'])
            ->setCellValue('AQ' . $row, $egresos_dato['fyh_actualizacion_detalle'])
            ->setCellValue('AR' . $row, $egresos_dato['nombre_usuario'] . " " . $egresos_dato['apaterno_usuario'] . " " . $egresos_dato['amaterno_usuario']);
        ...
        $value = $egresos_dato['saldo_inicial'];
        $spreadsheet->getActiveSheet()->setCellValue('D' . $row, $value);
        $spreadsheet->getActiveSheet()->getStyle('D' . $row)
            ->getNumberFormat()
            ->setFormatCode($currencyMask);

        $value = $egresos_dato['monto'];
        $spreadsheet->getActiveSheet()->setCellValue('AB' . $row, $value);
        ...
        $value = $egresos_dato['saldo'];
        $spreadsheet->getActiveSheet()->setCellValue('AE' . $row, $value);
        ...
        $dateValue = \PhpOffice\PhpSpreadsheet\Shared\Date::PHPToExcel($value);
        $dateValue = str_replace('FALSO', '', $dateValue);
        $spreadsheet->getActiveSheet()->setCellValue('AH' . $row, $dateValue);
        ...
        $dateValue = \PhpOffice\PhpSpreadsheet\Shared\Date::PHPToExcel($value);
        $dateValue = str_replace('FALSO', '', $dateValue);
        $spreadsheet->getActiveSheet()->setCellValue('S' . $row, $dateValue);
        ...
        $dateValue = \PhpOffice\PhpSpreadsheet\Shared\Date::PHPToExcel($value);
        $dateValue = str_replace('FALSO', '', $dateValue);
        $spreadsheet->getActiveSheet()->setCellValue('U' . $row, $dateValue);
        $spreadsheet->getActiveSheet()->getStyle('U' . $row)
            ->getNumberFormat()
            ->setFormatCode('dd/mm/YYYY');

        $value = $egresos_dato['fecha_informe_ingresos'];
        $dateValue = \PhpOffice\PhpSpreadsheet\Shared\Date::PHPToExcel($value);
        $dateValue = str_replace('FALSO', '', $dateValue);
        ...
        $value = $egresos_dato['fyh_creacion_detalle'];
        $dateValue = \PhpOffice\PhpSpreadsheet\Shared\Date::PHPToExcel($value);
        $dateValue = str_replace('FALSO', '', $dateValue);
        $spreadsheet->getActiveSheet()->setCellValue('AP' . $row, $dateValue);
        $spreadsheet->getActiveSheet()->getStyle('AP' . $row)
            ->getNumberFormat()
        ...
        $dateValue = \PhpOffice\PhpSpreadsheet\Shared\Date::PHPToExcel($value);
        $dateValue = str_replace('FALSO', '', $dateValue);
        $spreadsheet->getActiveSheet()->setCellValue('AQ' . $row, $dateValue);
        $spreadsheet->getActiveSheet()->getStyle('AQ' . $row)
            ->getNumberFormat()
            ->setFormatCode('dd/mm/YYYY HH:mm:ss');
        ...
        if ($row % 2 == 0) {
            //even row
            $spreadsheet->getActiveSheet()->getStyle('A' . $row . ':AR' . $row)->applyFromArray($evenRow);
            $spreadsheet->getActiveSheet()->getStyle('A' . $row . ':AR' . $row)->applyFromArray($styleArray);
            $spreadsheet->getActiveSheet()->getStyle('A' . $row . ':AR' . $row)->getAlignment()->setHorizontal('left');
        } else {
            //odd row
            $spreadsheet->getActiveSheet()->getStyle('A' . $row . ':AR' . $row)->applyFromArray($oddRow);
            $spreadsheet->getActiveSheet()->getStyle('A' . $row . ':AR' . $row)->applyFromArray($styleArray);
            $spreadsheet->getActiveSheet()->getStyle('A' . $row . ':AR' . $row)->getAlignment()->setHorizontal('left');
        }
        //increment row
    ...
    $lastRow = $row - 1;
    //set the autofilter
    $spreadsheet->getActiveSheet()->setAutoFilter("A" . $firstRow . ":AR" . $lastRow); //Establecer filtros en las columnas
}
