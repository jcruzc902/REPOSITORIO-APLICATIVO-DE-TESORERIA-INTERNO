...
foreach ($detalle_devolucion_datos as $detalle_devolucion_dato) {
    ...
    WHERE t_detalle.visible!=1 AND t_detalle.nombre_solicitante='$solicitante'";
    ...
    foreach ($detalle_dev as $detalle_persona) {
        ...
        $pdf->SetFillColor(255, 179, 3); // establece el color del fondo de la celda (en este caso es AZUL
    ...
    $pdf->SetFont('Times', '', 11); //fuente
    $pdf->Cell(15); //mover a la derecha
    $pdf->Cell(40, 10, utf8_decode('La devolución es por el monto de S/. ' . $monto_voucher . ' soles, y se afectaría a la Cta.Cte. N° ' . $nro_cuenta_banco)); //texto
...
$pdf->Cell(40, 10, utf8_decode('Área de Programación de Ingresos')); //texto

$pdf->Ln(0); //salto de linea
$pdf->SetFont('Times', '', 8); //fuente
$pdf->Cell(113); //mover a la derecha
$pdf->Cell(40, 10, utf8_decode('Jefe del Área de Programación de Ingresos')); //texto
