...
if ($action == 'ajax') {
	...
	$periodo6 = intval($_REQUEST['periodo6']);
	$txt_mes = array(
		"1" => "Ene",
		"2" => "Feb",
		"3" => "Mar",
	...
	function cantidad($table, $mes, $periodo6)
	{
		global $con;
		$fecha_inicial = "$periodo6-$mes-1";
		...
		$fecha_final = "$periodo6-$mes-$dia_fin";
	...
	$categorias[] = array('Mes', "EGRESOS - $periodo6");//Nombre de la primera y segunda fila del grafico
	for ($inicio = 1; $inicio <= 12; $inicio++) {
		$mes = $txt_mes[$inicio];//Obtengo la abreviatura del mes
		$cuenta1 = cantidad('egresos', $inicio, $periodo6);//Obtengo el  cantidad de vista_cuenta1
